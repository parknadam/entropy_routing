...
class PassLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Tensor만 반환 (transformers loop 호환)
...
# -----------------------------
# DecoderLayer import (Mistral/LLaMA)
# -----------------------------
def _get_decoder_layer_class(model_type: str):
    """모델 타입에 맞는 DecoderLayer 클래스 반환"""
    ...
    raise RuntimeError(f"Cannot import DecoderLayer for {model_type}")
...
class DynamicStageManager:
    def __init__(
        ...
        self.DecoderLayer = _get_decoder_layer_class(self.model_type)
    ...
    def _restore_one_layer(self, layer_i: int):
        ...
        # 새 디코더 레이어 생성
        try:
            new_layer = self.DecoderLayer(self.model.config, layer_i)
        except TypeError:
            new_layer = self.DecoderLayer(self.model.config)
    ...
    def _pass_one_layer(self, layer_i: int):
        old = self.layers[layer_i]
        self.layers[layer_i] = PassLayer().to(self.device)
        del old
    ...
    def set_stage(self, stage: str):
        ...
        for i in self.removed:
            cur = self.layers[i]
            is_pass = isinstance(cur, PassLayer)
...
def main():
    ...
    ap.add_argument("--base_model", required=True, help="A 모델 경로")
    ap.add_argument("--bundles_dir", required=True, help="bundles dir (B/, C/)")
    ...
    ap.add_argument("--thr_A_last_ent", type=float, default=2.5)
    ap.add_argument("--thr_A_ppl", type=float, default=40.0)
    ap.add_argument("--thr_AB_ent_mean", type=float, default=2.7)
    ap.add_argument("--thr_AB_last_ent", type=float, default=2.6)
