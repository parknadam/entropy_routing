...
def _rehydrate_layers(model, bundle_dir: str, indices: List[int]):
    ...
    for i in indices:
        try:
            new_layer = MistralDecoderLayer(model.config, layer_idx=int(i)).to(device=device, dtype=dtype)
        except TypeError:
            new_layer = MistralDecoderLayer(model.config).to(device=device, dtype=dtype)
...
def parse_args():
    ...
    p.add_argument("--teacher_model", default="mistralai/Mistral-7B-Instruct-v0.2")
...
def main():
    ...
    if args.stage == 1:
        ...
        bad = [i for i in AB_idx if (MistralDecoderLayer is not None and not isinstance(layers[i], MistralDecoderLayer))]
        if bad:
