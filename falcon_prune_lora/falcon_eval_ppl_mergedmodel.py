...
# 원본 모델과 비교
python -m falcon_prune_lora.falcon_eval_ppl_mergedmodel \
  --model_path tiiuae/falcon-7b ./merged_models_falcon/A_merged \
...
GET_LOADERS = None
for _mod_name in ["falcon_prune_lora.pruning.data", "pruning.data"]:
    try:
...
def _strip_layer_prefix(sd: Dict[str, torch.Tensor], layer_idx: int) -> Dict[str, torch.Tensor]:
    prefixes = [
        ...
        f"base_model.model.transformer.h.{layer_idx}.",
    ]
    out = {}
    for k, v in sd.items():
        for p in prefixes:
...
class DynamicStageManager:
    ...
    def _pass_one_layer(self, layer_i):
        old = self.layers[layer_i]
        self.layers[layer_i] = FalconPassLayer(return_tuple=self.passlayer_return_tuple).to(self.device)
        del old
...
Examples:
  ...
  # 원본 모델과 비교
  python -m falcon_prune_lora.falcon_eval_ppl_mergedmodel \\
    --model_path tiiuae/falcon-7b ./merged_models/A_merged \\
    --tokenizer_path tiiuae/falcon-7b
    ...
    for model_idx, mpath in enumerate(full_model_paths):
        ...
        for stage in stage_list:
            ...
            if stage in ("AB", "FULL") and mgr:
                ...
                mgr.set_stage(stage)
                n_active = mgr.num_layers - sum(1 for i in mgr.removed if isinstance(mgr.layers[i], FalconPassLayer))
