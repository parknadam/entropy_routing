# bundler.py  (Phi-3 전용)
# 점진적 프루닝을 위해 제거한 레이어를 B/C 두 번들로 나누어 저장
# ─────────────────────────────────────────────────────────────
# 변경 사유:
#   1. is_opt 파라미터 완전 제거
#      → _get_layers()에서 model.model.layers 직접 반환
#      → export_layer_bundle, export_two_bundles 시그니처 단순화
#   2. 나머지 로직(split, 파일 저장, 검증)은 모델 아키텍처에 무관하므로 동일 유지
# ─────────────────────────────────────────────────────────────
...
def _get_layers(model):
    """
    변경점: is_opt 분기 제거 → Phi-3는 항상 model.model.layers
    """
...
def _bundle_meta(config: PretrainedConfig, indices: List[int]) -> Dict:
    return {
        ...
        "arch": getattr(config, "model_type", "phi3"),  # 변경: 기본값 "phi3"
...
def export_layer_bundle(
    ...
    """
    지정된 레이어 인덱스들의 파라미터를 레이어별 safetensors 파일로 저장.
...
def export_two_bundles(
    ...
    """
    removed_indices를 B/C로 분할 후 각각 저장.
