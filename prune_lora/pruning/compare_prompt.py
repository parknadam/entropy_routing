# prune_lora/pruning/compare_prompt.py
"""
# 4개 변형(A, A+adapter, AB, FULL) 한 번에 출력

python -m prune_lora.pruning.compare_prompt \
  ...
  --a_adapter ./lora_results/adapters/A_lora/stageA/stageA
...
python -m prune_lora.pruning.compare_prompt \
  ...
  --device cuda:0 --dtype bf16 \
  --max_length 2048 --max_new_tokens 128 \
  --a_adapter ./kd_result3/adapters/A_lora/stageA/stageA
...
def _get_llama_layers(model) -> nn.ModuleList:
    ...
    # peft wrapper일 경우: base_model 내부를 먼저 탐색
...
def _maybe_shift_to_zero_based(
    B: Dict[int, Path], C: Dict[int, Path], num_layers: int
...
class DynamicStageManager:
    def __init__(self, model, bundles_dir: Path, device: str, dtype: torch.dtype, pass_return_tensor: bool):
        ...
        self.B_map, self.C_map, self.index_shift = _maybe_shift_to_zero_based(
            self.B_map_raw, self.C_map_raw, self.num_layers
        )
...
def _encode(tokenizer, prompt: str, device: str, max_length: int, chat: bool, system_prompt: str, add_generation_prompt: bool):
    if chat and hasattr(tokenizer, "apply_chat_template"):
        ...
        # 1) 최신 transformers: tokenize=True + return_tensors 지원
        try:
            ...
            # 2) 구버전: 문자열만 반환 → tokenizer로 다시 인코딩
        ...
        # out 타입 정리(버전별로 Tensor / BatchEncoding / list 다 나올 수 있음)
...
def _attach_adapter(model, adapter_dir: str):
    ...
    if PeftModel is None:
        raise RuntimeError("peft import 실패: `pip install peft` 필요")
    peft_model = PeftModel.from_pretrained(model, adapter_dir, is_trainable=False)
    peft_model.eval()
    return peft_model


def _run_one(name: str, model, tok, mgr: DynamicStageManager, stage: str, prompt: str, args, chat: bool, adapter_dir: Optional[str]):
    print("\n" + "=" * 90)
    print(f"[{name}]  stage={stage}" + (f"  adapter={adapter_dir}" if adapter_dir else ""))

    mgr.set_stage(stage)

    run_model = model
    if adapter_dir:
        run_model = _attach_adapter(model, adapter_dir)

    t0 = time.perf_counter()
    ppl = prompt_ppl(run_model, tok, prompt, args.device, args.max_length, chat, args.system_prompt)
    t1 = time.perf_counter()

    t2 = time.perf_counter()
    out = generate(run_model, tok, prompt, args.device, args.max_length, args.max_new_tokens, chat, args.system_prompt)
...
def main():
    ...
    # adapters
    ap.add_argument("--a_adapter", type=str, default=None, help="A 단계용 LoRA adapter 경로 (예: .../stageA)")
    ap.add_argument("--ab_adapter", type=str, default=None, help="AB(A+B) 단계용 LoRA adapter 경로 (옵션)")
    ap.add_argument("--full_adapter", type=str, default=None, help="FULL(A+B+C) 단계용 LoRA adapter 경로 (옵션)")
    args = ap.parse_args()
    ...
    prompt = args.prompt.strip() if args.prompt is not None else input("PROMPT> ").strip()

    print("=== CONFIG ===")
    print(f"base_model={args.base_model}")
    print(f"bundles_dir={args.bundles_dir}")
    print(f"device={args.device} dtype={args.dtype} chat={chat}")
    print(f"a_adapter={args.a_adapter}")
    print(f"ab_adapter={args.ab_adapter}")
    print(f"full_adapter={args.full_adapter}")

    bundles_dir = Path(args.bundles_dir)

    # -----------------------------
    # (1) A, (2) A + A-adapter  -> 같은 모델로 연속 수행 후 모델 폐기(오염 방지)
    # -----------------------------
    ...
    _run_one("A", model, tok, mgr, "A", prompt, args, chat, adapter_dir=None)

    if args.a_adapter:
        _run_one("A + A-adapter", model, tok, mgr, "A", prompt, args, chat, adapter_dir=args.a_adapter)
    else:
        print("\n" + "=" * 90)
        print("[A + A-adapter]  (skip) --a_adapter 미지정")
    ...
    # -----------------------------
    # (3) AB(A+B), (4) FULL(A+B+C)
    #   - 어댑터가 없으면 같은 모델에서 stage만 바꿔서 2번 실행
    #   - ab/full 어댑터가 지정되어 있으면 각자 깨끗한 모델로 실행(안전)
    # -----------------------------
    def run_stage_with_optional_adapter(name: str, stage: str, adapter_dir: Optional[str]):
        m = _load_model(args.base_model, dtype=dtype, device=args.device)
        m.eval()
        mg = DynamicStageManager(
            model=m,
            bundles_dir=bundles_dir,
            device=args.device,
            dtype=dtype,
        ...
        _run_one(name, m, tok, mg, stage, prompt, args, chat, adapter_dir=adapter_dir)
        del mg
        del m
        if str(args.device).startswith("cuda"):
            torch.cuda.empty_cache()

    if args.ab_adapter or args.full_adapter:
        # stage별로 깨끗하게 분리 실행
        run_stage_with_optional_adapter("A + B (AB)", "AB", args.ab_adapter)
        run_stage_with_optional_adapter("A + B + C (FULL)", "FULL", args.full_adapter)
    else:
        # adapter 없으면 한 번만 로드해서 stage switching
        model2 = _load_model(args.base_model, dtype=dtype, device=args.device)
        model2.eval()
        mgr2 = DynamicStageManager(
            model=model2,
            bundles_dir=bundles_dir,
            device=args.device,
            dtype=dtype,
        ...
        _run_one("A + B (AB)", model2, tok, mgr2, "AB", prompt, args, chat, adapter_dir=None)
        _run_one("A + B + C (FULL)", model2, tok, mgr2, "FULL", prompt, args, chat, adapter_dir=None)
