# prune_lora/pruning/eval_ppl.py
# Evaluate perplexity of A / AB / ABC(FULL) on a corpus (not user prompts).
#
# Examples
# 1) Using internal get_loaders (recommended if your repo already has it):
""" 
...
# -----------------------------
# Loader normalization (핵심: dict/tuple/tensor/str 모두 처리)
# -----------------------------
...
def _extract_batch(
    ...
    if isinstance(batch, dict):
        if "input_ids" in batch:
            ...
            if not torch.is_tensor(input_ids):
                return None
            input_ids = _ensure_2d(input_ids).to(device)

            if attn is None:
                attn = torch.ones_like(input_ids, dtype=torch.long)
            else:
                attn = _ensure_2d(attn).to(device)

            out = {"input_ids": input_ids, "attention_mask": attn}
            if labels is not None and torch.is_tensor(labels):
                out["labels"] = _ensure_2d(labels).to(device)
            return out

        # dict인데 input_ids가 없으면 스킵
        return None
    ...
    if torch.is_tensor(batch):
        input_ids = _ensure_2d(batch).to(device)
        attn = torch.ones_like(input_ids, dtype=torch.long)
        return {"input_ids": input_ids, "attention_mask": attn}

    # 4) string(라인 텍스트) 자체는 여기서 바로 배치로 못 만들고, 상위 packer에서 처리해야 함
    return None
...
def _normalize_loader_to_batches(
    ...
    """
    # raw_loader 자체가 str(전체 텍스트)인 경우
    ...
    # peek first to decide if it's str-iter or tensor/dict batches
    try:
        first = next(it)
    except StopIteration:
        return iter([])

    # if first is str => pack lines
    ...
    # otherwise, treat as batch-like iterator
    def _batches():
        made = 0
        # include first
...
def _pick_split(obj: Any, split: str) -> Any:
    ...
    split 선택 결과가 str로 떨어지면(이번 에러 원인) 다른 후보로 스왑 시도.
    """
    ...
    if isinstance(obj, tuple) and len(obj) >= 2:
        ...
        # cand가 str인데 other는 str이 아니면 스왑
...
def main():
    ...
    for stage, label in [("A", "A"), ("AB", "AB"), ("FULL", "ABC")]:
        ...
        loader = _normalize_loader_to_batches(
        ...
        m = eval_ppl(model, loader)
        print(f"[{label}] ppl={m['ppl']:.6f} | mean_nll={m['mean_nll']:.6f} | tokens={m['tokens']}")
