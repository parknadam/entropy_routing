...
# 원본 모델과 비교
python -m llama_prune_lora.eval_ppl_mergedmodel \
  --model_path meta-llama/Llama-2-7b-hf ./merged_models_llama_7b/A_merged \
...
try:
    from transformers.models.llama.modeling_llama import LlamaDecoderLayer as _LDL
    LlamaDecoderLayer = _LDL
except ImportError:
    pass

GET_LOADERS = None
for _mod_name in ["llama_prune_lora.pruning.data", "pruning.data"]:
    try:
...
def _read_bc_indices(model_path: str) -> Tuple[List[int], List[int]]:
    ...
    try:
        ...
        return [], []
    stages = manifest.get("stages", {})
    b_idx = sorted(set(int(i) for i in stages.get("B", {}).get("removed_layers", [])))
    c_idx = sorted(set(int(i) for i in stages.get("C", {}).get("removed_layers", [])))
    return b_idx, c_idx
...
def _install_passlayers(model, dropped_indices: List[int], return_tuple: bool = True):
    ...
    for idx in dropped_indices:
        if 0 <= idx < len(layers):
            ...
            layers[idx] = LlamaPassLayer(return_tuple=return_tuple).to(dev)
            del old
    print(f"  [ok] PassLayer installed: {dropped_indices} ({len(dropped_indices)} layers)")
    return model
...
def _load_model(model_path: str, dtype: torch.dtype, device: str):
    ...
    if dropped:
        print(
            f"  [ok] {model.config.model_type} | {n_layers} layers "
            f"({n_active} active + {len(dropped)} PassLayer) | {n_params/1e6:.1f}M params | {device}"
        )
    else:
        print(f"  [ok] {model.config.model_type} | {n_layers} layers | {n_params/1e6:.1f}M params | {device}")
    return model
...
class DynamicStageManager:
    ...
    def __init__(
        ...
        if LlamaDecoderLayer is None:
            raise RuntimeError("LlamaDecoderLayer import failed")
        if load_file is None:
            raise RuntimeError("safetensors import failed (pip install safetensors)")
    ...
    def set_stage(self, stage: str):
        ...
        for i in self.removed:
            is_pass = isinstance(self.layers[i], LlamaPassLayer)
            if i in pass_set:
                if not is_pass:
...
def prepare_batches(raw_loader, tok, seqlen, batch_size, device, max_batches):
    """BatchEncoding/TokenizerWrapper를 포함한 다양한 입력을 배치로 정규화."""
    ...
    if not batches:
        print(f"  [error] type={type(raw_loader).__name__}, batch extraction failed")
    return batches
...
def _apply_lora_no_merge(model, adapter_path: str):
    if PeftModel is None:
        raise RuntimeError("peft import failed. pip install peft")
...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluate PPL for merged LLaMA models.",
...
Examples:
  ...
  python -m llama_prune_lora.eval_ppl_mergedmodel \
    ...
    if args.stages is not None:
        ...
        print("  (--stages omitted, bundles detected -> A,AB,FULL)")
    else:
        stage_list = ["A"]
    ...
    # auto classify bundle dirs accidentally passed in model_path
    full_model_paths = []
    auto_b = None
    auto_c = None
    ...
    if has_bundles and stage_list == ["A"] and args.stages is None:
        ...
        print(f"  (bundles detected -> auto stages: {stage_list})")
    ...
    print("\n" + "=" * 70)
    print("Merged Model PPL Evaluation (LLaMA)")
    print("=" * 70)
    ...
    for model_idx, mpath in enumerate(full_model_paths):
        ...
        print(f"  [ok] {len(batches)} batches (seqlen={args.seqlen})")
        if len(batches) == 0:
            print("  [error] 0 batches")
            continue

        # model load (once per path), stage switching for reuse
        print("\n  Loading model...")
        model = _load_model(mpath, dtype=dtype, device=args.device)

        # stage manager only when needed
        mgr = None
        ...
        if needs_mgr:
            ...
            if not mgr.removed:
                b_manifest, c_manifest = _read_bc_indices(mpath)
                print(
        ...
        for stage in stage_list:
            ...
            if stage in ("AB", "FULL") and mgr:
                ...
                mgr.set_stage(stage)
                n_active = mgr.num_layers - sum(1 for i in mgr.removed if isinstance(mgr.layers[i], LlamaPassLayer))
