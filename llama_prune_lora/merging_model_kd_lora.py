...
python -m prune_lora.merging_model_kd_lora \
  --base_dir ./results/merged_models/A_merged \
  --bundles_dir ./7b_results/pruning/bundles \
  --stage 2 \
...
#!/usr/bin/env python3
"""
Progressive KD-LoRA 학습 코드 (최종 완성 버전)
...
bundles/
├── B/
│   ├── bundle_meta.json  <- "indices": [21, 22, 23, 24]
│   ├── layer_021.safetensors
│   └── ...
└── C/
    ├── bundle_meta.json
    └── ...
...
사용법:
# Stage 2
CUDA_VISIBLE_DEVICES=0,1 python -m prune_lora.progressive_kd_lora \
  --base_dir ./merged_models/A_merged \
  --bundles_dir ./7b_results/pruning/bundles \
  --stage 2 \
  --out_adapters ./kd_lora_results/adapters \
...
#!/usr/bin/env python3
"""
Progressive KD-LoRA 학습 코드 (수정 버전)

Stage 1: Base model + A 그룹 → A adapter 생성
Stage 2: A_merged model + B 그룹 → B adapter 생성  
Stage 3: A_merged model + B_merged bundle + C 그룹 → C adapter 생성

주요 변경사항 (Stage 3):
- A_merged (config.json 포함) 로드
- B_merged (bundle 형태, bundle_meta.json만 존재) 레이어 복원
- C (bundle 형태) 레이어 복원
- C 레이어에만 LoRA 어댑터 학습

사용법:
# Stage 3 (A_merged + B_merged + C)
python progressive_kd_lora_fixed.py \
  --base_dir ./merged_models/A_merged \
  --bundles_dir ./7b_results/pruning/bundles \
  --stage 3 \
  --out_adapters ./kd_lora_results/adapters \
...
# 유틸리티: 레이어 관리
# ============================================================
CANON_PATH = "model.layers"  # LlamaForCausalLM.model.layers
...
def _get_layer_container(model):
    """모델에서 레이어 컨테이너 가져오기"""
    parent, attr = _resolve_attr_path(model, CANON_PATH)
...
def _load_bundle_indices(bundle_dir: str) -> List[int]:
    ...
    if not os.path.exists(meta_path):
        print(f"    ✗ bundle_meta.json not found")
        return []
    
    with open(meta_path, "r") as f:
        meta = json.load(f)
    
    # "indices" 키에서 레이어 인덱스 가져오기
...
def _extract_layer_sd(raw_sd: dict, idx: int) -> dict:
    ...
    prefixes = [f"model.layers.{idx}.", f"layers.{idx}."]
    for p in prefixes:
...
def _pick_layer_file(bundle_dir: str, idx: int) -> str:
    ...
    # layer_021.safetensors 형태
    p = os.path.join(bundle_dir, f"layer_{idx:03d}.safetensors")
    if os.path.exists(p):
        return p
    # 혹시 layer_21.safetensors인 경우도 방어
...
def _rehydrate_layers(model, bundle_dir: str, indices):
    """Pruned bundle에서 레이어 복원 (디바이스 처리 개선)"""
...
def enable_only_lora_params_on_layers(model, indices, adapter_name=None):
    ...
    for n, p in model.named_parameters():
        m = _LAYER_RE.search(n)  # r"\blayers\.(\d+)\."
        if not m:
            continue
        ...
        # 여러 어댑터가 공존할 수 있어서, adapter_name을 쓰면 더 안전
        if adapter_name is not None and (f".{adapter_name}." not in n):
            continue
...
class KDTrainer(Trainer):
    def __init__(self, *args, teacher_model=None, kd_alpha=0.5, temperature=2.0, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        transformers 최신 버전 대응: num_items_in_batch 인자 추가
        """
        labels = inputs.pop("labels")
        s_out = model(**inputs, output_hidden_states=False, return_dict=True)
        s_logits_full = s_out.logits
        
        if self.teacher is None or not self.args.do_train:
            s_logits = s_logits_full.view(-1, s_logits_full.size(-1))
            labels_flat = labels.view(-1)
            loss = F.cross_entropy(s_logits, labels_flat, ignore_index=-100)
            return (loss, s_out) if return_outputs else loss
        
        # Teacher 입력: teacher 디바이스로 이동
        teacher_device = next(self.teacher.parameters()).device
        teacher_inputs = {k: v.to(teacher_device) if torch.is_tensor(v) else v 
                         for k, v in inputs.items()}
        
        with torch.no_grad():
            t_out = self.teacher(**teacher_inputs, output_hidden_states=False, return_dict=True)
            t_logits_full = t_out.logits
        
        # teacher logits를 student device로 이동
        t_logits_full = t_logits_full.to(s_logits_full.device)
        ...
        t = t_logits_full[mask].float().clamp(-50, 50)
        s = s_logits_full[mask].float().clamp(-50, 50)
        y = labels_s
...
def parse_args():
    ...
    p.add_argument("--base_dir", type=str, required=True, help="Base model directory (A_merged for stage 3)")
    p.add_argument("--bundles_dir", type=str, required=True, help="Pruned bundles directory (for stage 1,2) or C bundle dir (for stage 3)")
    p.add_argument("--b_merged_dir", type=str, default=None, help="B_merged directory (for stage 3 only)")
    p.add_argument("--stage", type=int, required=True, choices=[1, 2, 3], help="Training stage")
...
def main():
    ...
    if args.stage == 1:
        ...
        try:
            ...
            print("[Device] Falling back to device_map='auto'")
            model = AutoModelForCausalLM.from_pretrained(
                args.base_dir,
                torch_dtype=torch.float16,
                device_map="auto",
    ...
    # ========================================
        ...
        enable_only_lora_params_on_layers(model, B_idx, adapter_name="stageB")  # B_idx만!
        ...
        print(f"Adapter saved: {adapter_out}")
        print(f"\n[Next Steps]")
        print(f"1. Save B_merged as bundle (B layers are already trained)")
        print(f"2. Run stage 3 with:")
        print(f"   --base_dir ./merged_models/A_merged \\")
        print(f"   (B layers will be loaded from bundle in stage 3)")
    ...
    # ========================================
        ...
        # 1. A_merged 모델 로드 (config.json 존재)
        print("\n[Step 1] Loading A_merged model...")
        try:
            model = AutoModelForCausalLM.from_pretrained(
                args.base_dir,  # A_merged 경로
                torch_dtype=torch.float16,
                device_map={"": "cuda:0"},
                trust_remote_code=True
            )
        ...
        enable_only_lora_params_on_layers(model, C_idx, adapter_name="stageC")  # C_idx만!
        ...
        print(f"\n[Final Step - Create Complete Model]")
        print(f"To create the final ABC_merged model:")
        print(f"1. Load A_merged model")
        print(f"2. Rehydrate B_merged layers from bundle")
        print(f"3. Merge C adapter:")
        print(f"   python merge_adapter.py \\")
        print(f"     --base_model {args.base_dir} \\")
        print(f"     --adapter_path {adapter_out} \\")
        print(f"     --output_dir ./merged_models/ABC_merged_final \\")
        print(f"     --rehydrate_bundles {B_bundle_dir}")
